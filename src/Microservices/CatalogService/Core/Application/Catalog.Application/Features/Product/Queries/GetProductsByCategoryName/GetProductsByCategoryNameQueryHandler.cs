...
public class GetProductsByCategoryNameQueryHandler : IRequestHandler<GetProductsByCategoryNameQuery, List<ProductDto>>
{
    ...
    public async Task<List<ProductDto>> Handle(GetProductsByCategoryNameQuery request,
        CancellationToken cancellationToken)
    {
        ...
        if (validationResult.Errors.Any())
        {
            throw new BadRequestException(
                $"Bad {nameof(GetProductsByCategoryNameQueryHandler).GetHandlerName("QueryHandler")} request",
                validationResult);
        }
